...
const followUser = async (req, res) => {
    try {
      ...
      if (!userId || !userToFollowId) {
        return res.status(400).json({ message: 'Debe proporcionar los IDs de usuario' });
      }
      ...
      await user.save();
      return res.status(200).json({ message: 'Has comenzado a seguir a este usuario' });
    } catch (error) {
      console.error(error);
...
const unfollowUser = async (req, res) => {
    try {
      ...
      user.following = user.following.filter((followedUserId) => followedUserId !== userToUnfollowId);
